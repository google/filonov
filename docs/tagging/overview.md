...
`media-tagger` supports different modes of tagging.
...
If you want to change the default prompt used when describing media you can specify `custom_prompt` parameter. `custom_prompt` can be a either prompt or a file path (local or remote) with `.txt` extension.
